...
resource "google_storage_bucket" "artifact_bucket" {
  name     = "identity-functions-${var.environment}-cwb03-dsgov-acme"
  location = var.region
...
resource "google_storage_bucket_object" "code_archive" {
  ...
  name   = "cwb03-identity-functions/${var.environment}/app.zip"
...
resource "google_cloudfunctions_function" "gcip_before_create" {
  ...
  source_archive_object        = "cwb03-identity-functions/${var.environment}/app.zip"
  trigger_http                 = true
  ...
  secret_environment_variables {
    ...
    secret     = "cwb03-self-signed-token-private-key"
    version    = "latest"
  }
  lifecycle {
